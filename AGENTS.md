...
## Testing Guidelines
Use `flutter_test` for widget and unit coverage. Place tests in the matching feature path and end filenames with `_test.dart`. Prefer behavior-driven test names such as `testWidgets('calls onTap when pressed', ...)`. Add or update tests for UI behavior, BLoC state changes, and parsing/database logic when those areas change.
